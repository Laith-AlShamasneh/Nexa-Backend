...
- **Platform ceiling**: unlimited by default. Kestrel's `MaxRequestBodySize` and the multipart form parser's `MultipartBodyLengthLimit` are both driven from `Storage:MaxUploadSizeBytes` in `WebApi/Program.cs` — unset (or ≤ 0) means no limit at either layer; set it to a positive byte count to impose one. The per-file-type `UploadPolicy` caps (5 MB for logos) remain the actual control for *this* endpoint regardless. When running under IIS/IIS Express (the "IIS Express" launch profile), IIS enforces its *own* separate ceiling (`requestFiltering/requestLimits/maxAllowedContentLength`, default ~28.6 MB) in front of Kestrel entirely — `WebApi/web.config` raises that to IIS's maximum (`uint.MaxValue`, ~4 GB; IIS has no literal "unlimited" setting). A request IIS rejects never reaches the app, which is why it surfaces as IIS's own HTML "413.1" page, not anything `GlobalExceptionHandler` sees.
