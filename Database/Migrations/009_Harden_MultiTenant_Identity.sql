/*
    ...
    - Every individual change below is independently idempotent (IF NOT EXISTS /
      COL_LENGTH guards), so the whole script is safe to re-run in full.
    - GO batch separators are inserted after every ALTER TABLE ... ADD <column>
      whose new column is referenced by a later statement (FK, CHECK, INDEX, or
      DML) in this script. SQL Server compiles a batch's statements against the
      catalog state at the start of that batch, so a column added earlier in the
      SAME batch is not visible to a later statement in that same batch — see
      docs/DOMAIN_MODEL.md and the Phase 0 cleanup notes. This replaces the
      original single BEGIN TRY/CATCH structure, since TRY/CATCH cannot span GO
      batches: SET XACT_ABORT ON (below) provides the equivalent all-or-nothing
      guarantee instead — any runtime error in any batch auto-rolls-back the
      still-open transaction and aborts the script.
...
BEGIN TRANSACTION Migration009;
...
---------------------------------------------------------------------------
IF NOT EXISTS (SELECT 1 FROM dbo.SchemaVersions WHERE MigrationId = N'009')
BEGIN
...
COMMIT TRANSACTION Migration009;
